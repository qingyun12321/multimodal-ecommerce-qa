...
from ecom_qa.common.paths import artifacts_root, data_root, generated_reports_root
from ecom_qa.datasets.catalog import build_subcategory_queries, load_products
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--artifact-dir", type=Path, default=artifacts_root() / "runs")
    parser.add_argument(
        "--models",
...
def build_model_configs(args: argparse.Namespace) -> list[ModelRunConfig]:
    from ecom_qa.retrieval.models import ModelRunConfig
...
def main() -> None:
    args = parse_args()
    from ecom_qa.retrieval.models import build_retriever

    env_updates = ensure_workspace_env()
    device = resolve_device(args.device)

    products = load_products(args.data_dir)
    image_paths = [args.data_dir / product.image_path for product in products]
    ...
    write_json(
        ...
        {
            "retrieval_target": "text-to-image multilingual comparison",
