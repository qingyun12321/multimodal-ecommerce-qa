...
SFT multi-turn data generation reads tool-call splits from `data/tool_call/` and writes ms-swift JSONL under `data/training/sft/generated/mimo_multiturn/` by default. The SFT builder calls OpenRouter `xiaomi/mimo-v2.5` through the Python API to fill tag-free JSON fields; repository scripts own rendering of `<Think>`, `<Answer>`, `<RAG_search>`, `<Web_search>`, `<Grounding>`, and `<information>` tags. RAG turns can continue into Web_search when local retrieval is insufficient, and only the Web turn may end with the fixed unable-to-answer text. Web_search evidence comes from self-hosted SearXNG plus BGE-M3 hybrid scoring and BGE reranking, and final Web `<information>` rows contain content only, without title/source URL fields.
